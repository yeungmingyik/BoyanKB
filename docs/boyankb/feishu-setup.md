...
3. 在“添加应用能力”中添加“机器人”。该能力用于通过专用群组授予空间读取权限。
...
3. 核对机器人能力及读取权限。应用可用范围包含配置人员、知识空间管理员及相关资料所有者。
4. 保存并申请线上发布，由企业应用管理员审核。管理员按企业设置在“管理后台 → 工作台 → 应用审核”或飞书审批中处理。
5. 确认正式版本已经生效后再添加应用机器人。后续新增权限或变更应用能力时重新创建版本并发布。
...
2. 在群设置中添加刚发布的 `BoyanKB 知识同步` 应用机器人。
3. 打开目标知识空间的“知识库设置 → 成员设置”。
4. 选择“可阅读的成员”，搜索该专用群并添加。
5. 检查具有独立权限的页面、嵌入资源及附件，保证同步应用具有所需阅读和下载权限。

群中添加的是企业应用机器人；Webhook 自定义机器人不能提供这里的应用身份。空间的可阅读成员足以作为只读接入起点，无需授予空间管理员或编辑权限。[知识库应用授权](https://open.feishu.cn/document/server-docs/docs/wiki-v2/wiki-qa)

文档菜单“添加文档应用”授权的是该节点及其子节点，不能替代整空间授权。伙伴使用 BoyanKB 的账号授权，不加入这个群或飞书知识空间。

无法使用群组授权时，可由管理员使用其 `user_access_token`，通过成员接口将应用的 `open_id` 加为知识空间成员。该方式属于一次性管理员配置；同步应用不申请成员管理权限。应用 `open_id` 与 `cli_` 开头的 App ID 是不同标识。[成员授权方式](https://open.feishu.cn/document/server-docs/docs/wiki-v2/wiki-qa)、[成员接口](https://open.feishu.cn/document/server-docs/docs/wiki-v2/space-member/create)
