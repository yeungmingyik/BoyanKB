...
`0.1.0-alpha.4` 开发中：关键词与语义检索、知识问答、版本引用与在途撤权。

`0.1.0-alpha.3` 预发布版：已交付飞书整空间同步、系统内资料阅读与原生索引接入。首次完整发现 18 份资料，12 份已发布，6 份保持未发布；最终镜像的 HTTP、Worker、正式部署阅读与索引核对通过。实际覆盖见[资料覆盖清单](docs/boyankb/source-coverage.md)。
...
本版本用于本地集成验证。完整资料格式覆盖、关键词与语义搜索、企业知识问答尚未交付。开发计划见 [路线图](docs/boyankb/roadmap.md)，验证范围见 [验收清单](docs/boyankb/acceptance.md)，变更见 [更新日志](CHANGELOG.md)。
