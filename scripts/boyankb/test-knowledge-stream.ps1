...
$streamReport.metadata = @{
    ...
    hasSourceOverrides = @($streamInstance.Mounts | Where-Object { $_.Destination -match '^/app(?:$|/(?:packages|api|client|node_modules)(?:/|$))' -and $_.Destination -notmatch '^/app/client/public/images(?:/|$)' }).Count -gt 0
